...
BEGIN
	...
	LEFT OUTER JOIN TBL_FORMAT_COMMERCIAL_DOCUMENT FCD ON FCD.IDCOMPANY = SCD.IDCOMPANY AND FCD.TYPCOMDOC = SCD.TYPCOMDOC AND FCD.FORMAT = SCD.FORMAT
...
DROP FUNCTION IF EXISTS FN_PRINT_DOCUMENT_KARDEX;

DROP TYPE IF EXISTS REC_PRINT_DOCUMENT_KARDEX;

CREATE TYPE REC_PRINT_DOCUMENT_KARDEX AS (
	-- PRINT
	FORMAT VARCHAR(250),
	-- COMPANY
	COMDESTYPIDEDOC VARCHAR(250),
...
CREATE OR REPLACE FUNCTION FN_PRINT_DOCUMENT_KARDEX(
	IN_IDCOMPANY IN INTEGER,
	IN_CODUSER IN VARCHAR,
	IN_NUMINT IN BIGINT
)
RETURNS SETOF REC_PRINT_DOCUMENT_KARDEX AS
$BODY$
BEGIN
	RETURN QUERY
	SELECT
	...
	LEFT OUTER JOIN TBL_TYPE_INVENTORY TI ON TI.IDCOMPANY = DD.IDCOMPANY AND TI.TYPINV = DD.TYPINV
	LEFT OUTER JOIN TBL_ARTICLE A ON A.IDCOMPANY = DD.IDCOMPANY AND A.CODART = DD.CODART
	LEFT OUTER JOIN TBL_WAREHOUSE WO ON WO.IDCOMPANY = DD.IDCOMPANY AND WO.CODWAREHOUSE = DD.CODWAREHOUSE
	LEFT OUTER JOIN	TBL_WAREHOUSE WD ON WD.IDCOMPANY = DD.IDCOMPANY AND WD.CODWAREHOUSE = DD.CODWAREHOUSE
	WHERE
		DH.IDCOMPANY = IN_IDCOMPANY AND
		DH.NUMINT = IN_NUMINT
	ORDER BY
