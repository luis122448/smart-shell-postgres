...
DECLARE
	...
	V_LOG VARCHAR(1000);
BEGIN
	CALL PR_CREATE_GENERATOR_DOCUMENT(1, 'F002'::VARCHAR, 'A'::VARCHAR, 10, V_CODE, V_MESSAGE, V_LOG);
	RAISE NOTICE 'CODE: %', V_CODE;
    RAISE NOTICE 'MESSAGE: %', V_MESSAGE;
	RAISE NOTICE 'LOG: %', V_LOG;
END $$;
...
DECLARE
	...
	V_LOG VARCHAR(1000);
BEGIN
	CALL PR_CREATE_GENERATOR_DOCUMENT(1, 'B001'::VARCHAR, 'A'::VARCHAR, 10, V_CODE, V_MESSAGE, V_LOG);
	RAISE NOTICE 'CODE: %', V_CODE;
    RAISE NOTICE 'MESSAGE: %', V_MESSAGE;
	RAISE NOTICE 'LOG: %', V_LOG;
END $$;
...
DECLARE
	...
	V_LOG VARCHAR(1000);
BEGIN
	CALL PR_CREATE_GENERATOR_DOCUMENT(1, 'B002'::VARCHAR, 'A'::VARCHAR, 10, V_CODE, V_MESSAGE, V_LOG);
	RAISE NOTICE 'CODE: %', V_CODE;
    RAISE NOTICE 'MESSAGE: %', V_MESSAGE;
	RAISE NOTICE 'LOG: %', V_LOG;
END $$;
...
BEGIN
	...
	INSERT INTO TBL_DOCUMENT_HEADER (
		IDC0MPANY, NUMINT, CODEXT, TYPCOMDOC, SITCOMDOC, SERIE, NUMDOC, REGISTDATE, CODBRANCH, CODPLAISS, INGSALCOM, REACOMDOC,
...
BEGIN
	...
	IF NOT EXISTS (SELECT DH.NUMINT FROM TBL_DOCUMENT_HEADER DH WHERE DH.IDC0MPANY = IN_IDCOMPANY AND DH.NUMINT = IN_NUMINT ) THEN
		OUT_CODE := -2;
...
BEGIN
	...
	LEFT OUTER JOIN TBL_TYPE_COMMERCIAL_DOCUMENT TDC ON TDC.IDC0MPANY = DH.IDC0MPANY AND TDC.TYPCOMDOC = DH.TYPCOMDOC
	LEFT OUTER JOIN TBL_SITUATION_COMMERCIAL_DOCUMENT SDC ON SDC.IDC0MPANY = DH.IDC0MPANY AND SDC.TYPCOMDOC =  DH.TYPCOMDOC AND SDC.SITCOMDOC = DH.SITCOMDOC
	LEFT OUTER JOIN TBL_REASON_COMMERCIAL_DOCUMENT MDC ON MDC.IDC0MPANY = DH.IDC0MPANY AND MDC.TYPCOMDOC =  DH.TYPCOMDOC AND MDC.INGSALCOM = DH.INGSALCOM AND MDC.REACOMDOC =  DH.REACOMDOC
	LEFT OUTER JOIN TBL_SELLER S ON S.IDC0MPANY = DH.IDC0MPANY AND S.CODSEL = DH.CODSEL
	LEFT OUTER JOIN TBL_TYPE_PAYMENT_CONDITION TCP ON TCP.IDC0MPANY = DH.IDC0MPANY AND TCP.TYPPAYCON = DH.TYPPAYCON
	WHERE
		DH.IDC0MPANY = IN_IDCOMPANY AND
...
BEGIN
	...
	FROM TBL_DOCUMENT_HEADER DH
	LEFT OUTER JOIN TBL_SERIE_COMMERCIAL_DOCUMENT SCD ON SCD.IDC0MPANY = DH.IDC0MPANY AND SCD.TYPCOMDOC = DH.TYPCOMDOC AND SCD.SERIE = DH.SERIE
	LEFT OUTER JOIN TBL_COMPANY_INFO CI ON CI.IDC0MPANY = DH.IDC0MPANY
	LEFT OUTER JOIN TBL_TYPE_IDENTITY_DOCUMENT TID1 ON TID1.IDC0MPANY = DH.IDC0MPANY AND TID1.TYPIDEDOC = CI.TYPIDEDOC
	LEFT OUTER JOIN TBL_BUSINESS_PARTNER BP ON BP.IDC0MPANY = DH.IDC0MPANY AND BP.CODBUSPAR = DH.CODBUSPAR
	LEFT OUTER JOIN TBL_TYPE_IDENTITY_DOCUMENT TID2 ON TID2.IDC0MPANY = DH.IDC0MPANY AND TID2.TYPIDEDOC = BP.TYPIDEDOC
	LEFT OUTER JOIN TBL_TYPE_COMMERCIAL_DOCUMENT TDC ON TDC.IDC0MPANY = DH.IDC0MPANY AND TDC.TYPCOMDOC = DH.TYPCOMDOC
	LEFT OUTER JOIN TBL_SITUATION_COMMERCIAL_DOCUMENT SDC ON SDC.IDC0MPANY = DH.IDC0MPANY AND SDC.TYPCOMDOC =  DH.TYPCOMDOC AND SDC.SITCOMDOC = DH.SITCOMDOC
	LEFT OUTER JOIN TBL_REASON_COMMERCIAL_DOCUMENT MDC ON MDC.IDC0MPANY = DH.IDC0MPANY AND MDC.TYPCOMDOC =  DH.TYPCOMDOC AND MDC.INGSALCOM = DH.INGSALCOM AND MDC.REACOMDOC =  DH.REACOMDOC
	LEFT OUTER JOIN TBL_SELLER S ON S.IDC0MPANY = DH.IDC0MPANY AND S.CODSEL = DH.CODSEL
	LEFT OUTER JOIN TBL_TYPE_PAYMENT_CONDITION TCP ON TCP.IDC0MPANY = DH.IDC0MPANY AND  TCP.TYPPAYCON = DH.TYPPAYCON
	LEFT OUTER JOIN TBL_DOCUMENT_DETAIL DD ON DD.IDC0MPANY = DH.IDC0MPANY AND DD.NUMINT = DH.NUMINT
	LEFT OUTER JOIN TBL_TYPE_INVENTORY TI ON TI.IDC0MPANY = DD.IDC0MPANY AND TI.TYPINV = DD.TYPINV
	LEFT OUTER JOIN TBL_ARTICLE A ON A.IDC0MPANY = DD.IDC0MPANY AND A.CODART = DD.CODART
	WHERE
		DH.IDC0MPANY = IN_IDCOMPANY AND
		DH.NUMINT = IN_NUMINT
	ORDER BY
